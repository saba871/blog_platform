...
const { blogPostIdeasPrompt, generateReplyPrompt, blogSummaryPrompt } = require('../utils/prompts')
...
const generateByBlogPost = async (req, res) => {
    try {
        ...
        if (!title || !tone) return res.status(400).json({ message: 'Missing require Fields' })
...
const generateBlogPostIdeas = async (req, res) => {
    try {
        ...
        if (!topics) return res.status(400).json({ message: 'Missing require fields' })
...
const generateCommentReply = async (req, res) => {
    try {
        ...
        if (!author || !content) return res.status(400).json({ message: 'Missing require fields' })
        ...
        res.status(500).json({ message: 'Failed to generate comment', error: error.message })
    }
}
...
const generatePostSummary = async (req, res) => {
    try {
        ...
        const fixedJson = jsonMatch[0].replace(/"(title|summary)":\s*"([\s\S]*?)"\s*(?=[,}])/g, (match, key, value) => {
