...
const PostByTags = () => {
	...
	const getActiveMenu = () => {
		const found = BLOG_NAVBAR_DATA.find(item => item.path === `/tag/${tagName}`)
		return found?.id || ''
	}
		...
		<BlogLayout activeMenu={getActiveMenu()}>
			...
			<div className="w-full bg-gradient-to-r from-sky-100 via-cyan-50 to-blue-100 rounded-2xl py-10 mb-8 flex flex-col items-center justify-center text-center">
				...
				<p className="text-sm text-gray-500">
					...
					{blogPostList.length !== 1 ? 's' : ''} tagged with <span className="font-medium">#{tagName}</span>
				</p>
			</div>
					...
					{blogPostList.length > 0 ? (
						...
						<div className="flex flex-col items-center justify-center py-20 text-center">
							<p className="text-gray-500 text-sm">
								No posts found for <span className="text-sky-500 font-medium">#{tagName}</span>
							</p>
						</div>
					)}
				</div>
