...
const CommentInfoCard = ({
  ...
  return (
    <div
      className={`group relative flex gap-3 ${!isSubReply ? 'py-5 border-b border-gray-100' : 'py-3'}`}
    >
      {/* Avatar */}
      <div className="flex-shrink-0 mt-0.5">
        <img
          src={authorPhoto}
          alt={authorName}
          className={`rounded-full object-cover ${isSubReply ? 'w-8 h-8' : 'w-10 h-10'}`}
        />
      </div>
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2.5">
            ...
            {replies?.length > 0 && (
              ...
              >
                ...
                <LuChevronDown
                  className={`text-[13px] transition-transform duration-200 ${showSubReplies ? 'rotate-180' : ''}`}
                />
              </button>
            )}
          </div>
        ...
        {onDelete && (
          <button
            className="absolute top-5 right-0 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-400 transition-all duration-150"
